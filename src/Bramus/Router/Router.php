...
final class Router
{
    ...
    public function before($methods, $pattern, $fn)
    {
        ...
        foreach (explode('|', $methods) as $method) {
            $this->_beforeRoutes[$method][] = array(
                'pattern' => $pattern,
                'fn' => $fn
            );
        }
     ...
     *
     * @param string $base_route The route sub pattern to mount the callable's on
     * @param callable $fn The method to be called
     */
    public function mount($base_route, $fn)
    {
        // track current base route
        $current_base_route = $this->_baseRoute;

        // build new base route string
        $this->_baseRoute .= $base_route;

        // call the desired method
        call_user_func($fn);

        // restore original base route
        $this->_baseRoute = $current_base_route;
    }
    ...
    {
        // check if php method getallheaders is built in current php distro
    ...
    public function run($callback = null)
    {
        ...
        if (isset($this->_routes[$this->_requestedMethod])) {
            $handled_count = $this->handle($this->_routes[$this->_requestedMethod], true);
        }
